...
corrPocketPairs_al <- function(x, ppm, ws, reg = NULL, plotHeatmap = FALSE, wdlimit = 0.99,
  ...
  for (i in notNoise) {
    ...
    bigEnough <- lapply(notcenter, function(x) {
      peaks$bounds[[x]] %>%
        unlist() %>%
        fillbetween() %>%
        cc[., i] %>%
  ...
  prs <- sortPairs(rbind(colInCC, colInPPM)) %>%
    matrix(., ncol = 2) %>%
    data.frame()
