...
matchPeaksToRef2 <- function(target, driver_ppm, reference, tol = 0.02,
  ...
  # Match using iterative minimum distance or global cluster
  # optimization (MTJ 2022)
  ...
  matchlist <- list(
    ref = list(
      ppms = reference[ref_inds, "chemical-shift"],
      intensities = reference[ref_inds, "intensity"],
      inds = ref_inds
    ),
    target = list(
      ppms = target[target_inds, "chemical-shift"],
