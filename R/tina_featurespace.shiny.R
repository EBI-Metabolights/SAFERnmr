...
# Prep the data ---- 
  # Convert to df outside of the plot_umap.scores function ####
      df <- data.frame(x = umap.obj$layout[,1], 
                       y = umap.obj$layout[,2],
...
# Build the app ----
  # Define UI ----
      ui = fluidPage(
        ...
        sidebarLayout(
          position = "right",
          sidebarPanel(h2("Extracted Features for selected points"),
          ...
          mainPanel(
            ...
            h4(stringr::str_c("Features in UMAP - n_neighbors = ", umap.obj$config$n_neighbors,
  ...
  # Define server logic ----
      server = function(input, output) {
        ...
        # Feature Plot
          # Try printing the indices first ####
          # selectedPoints <- 1:5
            # output$se <- renderPrint({
            #       # coords <- event_data("plotly_selected")
            #       
            #       # Identify points by coordinates (currently no other way to get IDs)
            #         # which((df$x %in% coords$x) & (df$y %in% coords$y))
            #         
