...
backfit_ref.feats.2.subset.specs <- function(m.inds, fits.feature, match.info,
  ...
  backfits <- pblapply(m.inds, function(m) {
    ...
    spec.cols <- match.info[m, c("feat.start", "feat.end")] %>%
      as.numeric() %>%
      feature$position[feat, .] %>%
      fillbetween()
    thisref.ppm <- ref.region %>%
      fillbetween() %>%
      ppm[.]
    ...
    ss <- feature$subset$ss.all[feat, ] %>% which()


    # Calculate fits to subset spectra: ####
    fits <- lapply(ss, function(ss.spec) {
      ...
      fit.ref <- fit.feat2spec$ratio * fit$spec.fit + fit.feat2spec$intercept
