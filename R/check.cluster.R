...
#' @importFrom magrittr %>%
    ...
    # Choose representative shape
      # Calculate all pairwise lsfs and rmses
          ...
          feature.stack <- feature.stack[clust.feats, ,drop = F] %>% trim.sides
      ...
      # Get the RMSE ####
          lag.table <- lapply(1:nrow(lag.table), function(m) {
            pair <- lag.table[m, ]
            al.fts <- lag.features(feature.stack, pair) %>% trim.sides # al.fts[2,] %>% simplePlot
            fit <- fit.leastSquares(al.fts[1,], al.fts[2,], plots = F, scale.v2 = T)
              # fit$plot %>% plot
            pair$rmse <- fit$rmse
            return(pair)
          }) %>% do.call(rbind,.)
      ...
      # Pick key feature by minimizing average rmse  ####
        ...
        #   - if you trim.sides, you lose any indexing within the pos.stack
        ...
        #     to be derived using trim.sides(out = "inds"), but this can be done if needed)
          al.fts <- lag.features(feature.stack, lag.table, to = key.feat) %>% trim.sides # %>% simplePlot

            # Calc fits ####
              fits.pw <- lapply(1:nrow(al.fts), function(m) {
                fit <- fit.leastSquares(al.fts[m, ],
                                        al.fts[key.feat, ],
                                        plots = F,
                                        scale.v2 = T)
                  # fit$plot %>% plot
              ...
              al.fts <- lapply(fits.pw, function(fit) fit$feat.fit) %>% do.call(rbind,.) %>% trim.sides
