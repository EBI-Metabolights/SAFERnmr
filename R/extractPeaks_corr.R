...
extractPeaks_corr <- function(corr, mask = NULL, plots = FALSE){
  ...
  if (is.matrix(corr)){corr <- as.numeric(corr)}
  if (is.null(mask)){mask <- !is.na(corr)}
              ...
              # Check extracted window by plotting. Color in these STOCSYs is abs(cc)
                if (plots){
                              ...
                              g <- ggplot(data = df, aes(x = inds, y = r)) +
                                ...
                                geom_point(data = data.frame(xs = localMaxs, ys = corr[localMaxs]),
                                       aes(x = xs, y = ys), na.rm = TRUE) +
                                # Indicate excluded points
                                  geom_point(data = data.frame(xs = excluded, ys = corr[excluded]),
                                             aes(x = xs, y = ys),
                                             shape=4, na.rm = TRUE) +
                                scale_x_reverse()
                              plot(g)
