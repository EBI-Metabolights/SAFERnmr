...
#'
#' This function matches STORM features to a reference database using cross-correlation,
...
#' a YAML file and data from RDS files generated by a previous function call.
#'
...
############ Match each feature to refs - v2 - "Par"ty time ####
    ...
    # Run the matching loop ####
        ...
        t1 <- Sys.time()
            matches <-  foreach(f.num = f.subset,
            ...
            # Evaluate fits for top-scoring positions (regardless of ref) ####
              ...
              # all(rowSums(allmatches.feat[,c('wasserstein.score','sum.residuals','rmse')]))
              # if (){return()}
