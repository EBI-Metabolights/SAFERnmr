...
stackplot <- function(ymat = NULL, xvect = NULL,
  ...
  if (is.vector(ymat)){ymat <- t(matrix(ymat))}
  
  # Shift by row number
  if (is.null(shift_by)){shift_by <- 1:nrow(ymat)}
  ...
  #   for each spectrum. No vshift needed.
    ...
    # Calculate actual hshift value to be fraction of on ppm range
        actualH <- xvect %>% range %>% diff %>% abs %>% "*" (hshift) 
        
    # Center each spectrum's median at the plot point
        # meds <- ymat %>% apply(.,1,median)
        # yshifts <- meds + shift_by * actualV
