...
pair.score.summation <- function(pars) {
  ...
  refmat <- lapply(lib.data.processed, function(x) x$mapped$data) %>%
    do.call(rbind, .) %>%
    t()
  ...
  if (foreach::getDoParRegistered()) {
    ...
    stop("pair.score.summation: Cluster initiaion failed.")
  }
