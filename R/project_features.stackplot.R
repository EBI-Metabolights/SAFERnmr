...
#' @param bestfits A list containing the best fit data for each feature, as generated by \code{\link{project_features.getBestFits}}
...
project_features.stackplot <- function(xmat, ppm,
  ...
  exp.ranges <- apply(bestfits$fit.positions, 1, function(x) {
    x %>%
      range(na.rm = TRUE) %>%
      ppm[.] %>%
      sort()
  })
  if (length(exp.ranges) == 0) {
    return(NULL)
  }
  lbound <- min(ppm)
  ubound <- max(ppm)
  ...
  plots <- lapply(plot.list, function(p) {
    list(
      rows = sort(p), # sort because max.cliques gives ~random order
  ...
  plots.all.ranges <-
    lapply(1:nplots, function(p) {
      ...
      if (length(rowinds) > 1) {
        ...
        x.rev <- xmat[rowinds, colinds, drop = F]
      }
      ...
      if (length(rowinds) > 1) {
        ...
        f.rev <- bestfits$fit.feats + v.adjustments
      }
      ...
      if (length(rowinds) > 1) {
        f.ppm <- bestfits$fit.positions[rev(plotn$rows), , drop = F] %>%
          apply(1, complete.indsVect) %>%
          t() %>%
          apply(., 1, function(x) ppm[x]) %>%
          t() + h.adjustments
      } else {
