...
#'
#' @author MTJ
apply.fit <- function(mi.row, feat.stack, ref.stack){
  # mi.row <- match.info[1,]
  
  v1 <- feat.stack[mi.row$feat, mi.row$feat.start:mi.row$feat.end] %>% scale.between
  v2 <- ref.stack[mi.row$ref.start:mi.row$ref.end, mi.row$ref] %>% scale.between
  # if (scaled){
  #  v1 <- v1 %>% scale.between
  #  v2 <- v2 %>% scale.between
  # }
