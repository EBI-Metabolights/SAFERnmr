...
matchCluster <- function(t, # target peaks
                      ...
                      method = "itmin"){ # simple iterated method ("itmin"), 
                                        # substructure-based ('substruct'), or optimal ("opt") - NOT IMPLEMENTED YET
...
## New code: 
  ...
  # Calculate distance matrix; threshold on tol
    ...
    # write.table(d,file = "d.csv",row.names = FALSE,col.names = FALSE)
  ...
  # Iterated minimum method
    ...
    while (any(!is.na(d)) & (length(na.omit(matched_targets)) <= possibleMatches)){
      ...
      # Check if there was even a min
        ...
        } else {break} # If there is nothing left in d
      ...
      # browser()
      # segmentRef_simpleTolerance(ref)
  ...
  # Remove NAs from the list and return
    # Add the indices
      return(list(matchedTargets = na.omit(matched_targets),
