    ...
    # Set up standard marker
       std.marker <- list(size = 7.5,
        ...
        fig <- plotly::plot_ly(data = data.frame(x = umap.obj$layout[,1],
                                           ...
                                           marker = std.marker) %>%
                           plotly::layout(title = str_c("Features in UMAP - n_neighbors = ", umap.obj$config$n_neighbors,
...
# Previous:
# plot_umap.scores <- function(scores, n_neighbors, min_dist){
#   
  # fig <- plotly::plot_ly(data = data.frame(x = scores[,1],
  #                                          y = scores[,2]),
  #                                    x = x, y = y,
  #                                    type="scatter", mode = "markers",
  #                                    marker = list(size = 7.5,
  ...
  #                                                  line = list(color = 'rgba(152, 0, 0, .8)',
  #                                                              width = 2))) %>%
  #                    plotly::layout(title = str_c("Features in UMAP - n_neighbors = ", n_neighbors,
