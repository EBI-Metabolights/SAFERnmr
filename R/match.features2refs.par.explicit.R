...
#' @return A list of matches between features and reference spectra.
#' @export
#'
#' @import parallel
#' @import foreach
...
match.features2refs.par.explicit <- function(pars) {
    ...
    log_path <- paste0("/nfs/production/odonovan/nmr_staging/debug_matching_files/readouts/", Sys.time())
    dir.create(log_path)
    ...
    matches.all <- foreach(
        ...
        #mem.snapshot(paste0("/nfs/production/odonovan/nmr_staging/debug_matching_files/readouts/", Sys.time(), "_chunk.txt"))
        ...
        matches.chunk <- foreach(
            f.num = f.subset, # what actually gets recorded
