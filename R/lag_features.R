...
#' (last 2 columns unused here, but are in the output from feat_align())
#' 
...
#' @param lag.table lag information table from feat_align or similar
...
#'
#' @export
lag_features <- function(featureStack,  # each row is a feature
  ...
  # Use the lag.table to adjust the relative alignments of the relevant rows of featureStack
    ...
    # Define relevant rows 
      ...
      # Pull out all matches involving the key feature ("to") ####
        ...
        hits <- pw_lags_relative_to(lag.table, feat.num = to)
