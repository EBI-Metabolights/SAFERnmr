...
tmp <- '/Users/mjudge/Documents/ftp_ebi/pipeline_runs_new/1709892382'
...
region <- c(1.676, 2.273)
samplenames <- colnames(scores$ss.ref.mat)
    select.sample <- grepl(pattern = 'ADG10003u_007', x = samplenames) %>% which
selectedCols <- select.sample # 92
ss.spec <- selectedCols %>% samples$id[.]
metab.evidence <- select_evidence_refmet(ref = selectedRow %>% refs[.,],
                                         sample = selectedCols %>% samples[.,],
                                         # Big objects to subset using ref.ind:
...
# look at one fit
  ...
  simplePlot(rbind(bfs$fit.feats,
                   xmat[bfs$fit.xrow, bfs$fit.positions %>% complete_indsVect]))
